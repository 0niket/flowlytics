...
function renderBasketCountExplanation(config: typeof builder.config): string {
  ...
  return `
    <details class="formula-box" style="text-align:left;font-size:11px;line-height:1.6;cursor:default;">
      <summary style="cursor:pointer;list-style:none;display:flex;align-items:center;gap:6px;">
        <span style="font-size:9px;color:var(--muted);transition:transform 0.15s;" class="formula-box__chevron">\u25b6</span>
        <span style="font-size:10px;color:var(--muted);">Why ${b.result}?</span>
        <span style="font-size:11px;font-family:var(--mono);color:var(--muted);">L = \u03bb \u00d7 W = ${b.result}</span>
      </summary>
...
function renderEconomicsSection(container: HTMLElement): void {
  const econ = builder.config.economics;
  ...
  section.innerHTML = `
    <summary class="config-section__header">
      <span class="config-section__title">Economics</span>
      <span class="badge badge--neutral config-section__summary" id="cvEconomicsSummary">${getEconomicsSummary()}</span>
    </summary>
    <div class="config-section__body">
      <fieldset class="economics-fieldset">
        <legend class="economics-fieldset__legend">Revenue</legend>
        <div class="field">
...
function wireListeners(signal: AbortSignal): void {
  document.addEventListener(
    ...
    (e) => {
      ...
      // Per-wagon cost
      if (target.classList.contains("bldr-wagon-cost")) {
        const idx = Number(target.getAttribute("data-wagon-index"));
        const val = Number((target as HTMLInputElement).value) || 0;
